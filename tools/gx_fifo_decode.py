...
class Decoder:
    def __init__(self, data, offset_pc):
        self.data = data
        self.n = len(data)
        self.offset_pc = offset_pc  # fn(offset) -> pc string
        self.cp = CPState()
        self.commands = []  # (offset, pc, opcode_name, detail, size)
        # aggregate inventory
        self.bp_writes = {}    # addr -> list of values (ordered unique kept small)
        self.cp_writes = {}
        self.xf_writes = {}    # addr -> list of value-tuples
        self.draws = []        # (prim, vat, count, vsize)
        self.calldls = []      # (offset, addr, size)
        self.nop_count = 0
        self.efb_copies = []   # snapshots of the copy cluster
        self.setdrawdone = []  # values
        self.pe_token = []
        self.pe_token_int = []
        # live BP cluster tracking for EFB copy snapshots
        self._bp_latch = {}

    def pc_at(self, off):
        return self.offset_pc(off)

    def emit(self, off, name, detail, size):
        self.commands.append((off, self.pc_at(off), name, detail, size))
    ...
    def decode_one(self, off):
        ...
        if op == GX_NOP:
            ...
            while off + count < n and data[off + count] == GX_NOP:
                count += 1
            self.nop_count += count
            self.emit(off, "NOP", "x%d" % count, count)
            return count, count
        ...
        if op == GX_LOAD_CP_REG:
            ...
            self.cp_writes.setdefault(cmd2, []).append(value)
            self.emit(off, "LOAD_CP_REG",
        ...
        if op == GX_LOAD_XF_REG:
            ...
            if (cmd2 >> 16) >= 16:
                raise DecodeError(
                    "XF stream_size field >=16 at offset %d (cmd2=0x%08X) pc=%s"
                    % (off, cmd2, self.pc_at(off)))
            total = 5 + stream_size * 4
            if off + total > n:
                return None, 0
            vals = [swap32(data, off + 5 + 4 * i) for i in range(stream_size)]
            self.xf_writes.setdefault(base_addr, []).append(tuple(vals))
            detail = self._xf_detail(base_addr, stream_size, vals)
            self.emit(off, "LOAD_XF_REG", detail, total)
            return total, total
        ...
        if op == GX_CMD_CALL_DL:
            ...
            self.calldls.append((off, address, size))
            self.emit(off, "CALL_DL",
                      "addr=0x%08X size=0x%X (contents in guest RAM, NOT in stream)"
                      % (address, size), 9)
            return 9, 9

        if op == GX_CMD_UNKNOWN_METRICS:
            self.emit(off, "UNKNOWN_METRICS", "(1 byte)", 1)
            return 1, 1

        if op == GX_CMD_INVL_VC:
            self.emit(off, "INVL_VC", "(1 byte, vertex cache invalidate)", 1)
            return 1, 1
        ...
        if op == GX_LOAD_BP_REG:
            ...
            self._on_bp(off, addr, value)
            self.emit(off, "LOAD_BP_REG",
        ...
        if GX_PRIMITIVE_START <= op <= GX_PRIMITIVE_END:
            ...
            if off + total > n:
                return None, 0
            self.draws.append((prim, vat, nverts, vsize))
            self.emit(off, "DRAW",
        ...
        raise DecodeError(
            "UNKNOWN opcode 0x%02X at offset %d (pc=%s) context=[..%s..]"
            % (op, off, self.pc_at(off), ctx))

    # ---- BP EFB-copy cluster tracking ----
    def _on_bp(self, off, addr, value):
        self.bp_writes.setdefault(addr, []).append(value)
        self._bp_latch[addr] = value
        if addr == 0x45:  # SETDRAWDONE
            self.setdrawdone.append((off, value))
        elif addr == 0x47:  # PE_TOKEN_ID
            self.pe_token.append((off, value))
        elif addr == 0x48:  # PE_TOKEN_INT_ID
            self.pe_token_int.append((off, value))
        elif addr == 0x52:  # TRIGGER_EFB_COPY
            self.efb_copies.append((off, self.pc_at(off), dict(self._bp_latch)))
